...
export const removePriceFromTotal = (
  totalCost: number,
  cartItemPrice: number
): number => {
  return totalCost - cartItemPrice;
...
export const removeItemFromCount = (
  cartItemCount: number,
  count: number
): number => {
  return cartItemCount - count;
...
export const getCartItemsCount = (cartItems: ItemProps[]) =>
  cartItems.reduce(
    (accumalatedQuantity: number, cartItem: ItemProps) =>
      accumalatedQuantity + cartItem.stock,
    0
  );
